...
export const BuyDiamondsSection = ({ className }: Props) => {
  return (
    ...
    >
      <img
        src={`${sectionDiamonds}?v=${Date.now()}`}
        alt="section diamonds"
        className="object-contain w-full max-w-[230px] md:max-w-none md:hidden"
      />
      ...
      <div className="mt-4 md:mt-0 flex flex-col-reverse md:flex-row items-center gap-4 md:gap-8 z-1 w-full md:w-auto">
        ...
        <div className="flex items-center gap-3 md:gap-8">
          {["Без комиссии", "Зачисление за 10 минут"].map((item) => (
            <div className="flex items-center p-3 w-auto md:px-8 md:py-4 rounded-full bg-[#127DC5]">
